...
class PickUp(Node):
    ...
    def marker_listener_callback(self, msg: MarkerPoseWithID):
        ...
        x, y, z, qw, qx, qy, qz = self.compute_target_pose(
                avg_pose,
                distance=0.160, #0.118
                x_offset=0.0,
                y_offset=0.0,
                z_offset=0.005, #0.10 #0.03
        )
        goal_pose = Pose()
    ...
    def compute_target_pose(
        ...
        # Another rotation of 10 degree around Y (original detected y)
        angle_rad = math.radians(10)
        half_angle = angle_rad / 2
        rotate_y_10 = np.quaternion(math.cos(half_angle), 0, math.sin(half_angle), 0)
        result_quat0 = rotate_y_10 * result_quat0
