...
class CartesianMoveNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('x_offset', 0.05)
    ...
    def get_current_pose_and_move(self):
        ...
        self.get_logger().info('Waiting 2s before requesting planning scene...')
        time.sleep(2.0)
        scene_req = GetPlanningScene.Request()
        scene_req.components.components = scene_req.components.ROBOT_STATE
        self.get_logger().info('Calling get_planning_scene...')
        future_scene = self.get_scene_cli.call_async(scene_req)
        rclpy.spin_until_future_complete(self, future_scene, timeout_sec=5.0)
        if not future_scene.done():
            self.get_logger().error('Timeout: get_planning_scene did not respond.')
            return
        self.get_logger().info('Received planning scene.')
        ...
        self.get_logger().info(f"Current EE pose X: {current_pose.position.x:.4f}")

        # === Step 3: Build Cartesian path request ===
        target_pose = copy.deepcopy(current_pose)
        target_pose.position.x = target_pose.position.x + self.x_offset
        ...
        self.get_logger().info(f"Target EE pose X: {target_pose.position.x:.4f}")

        self.get_logger().info('Getting cartesian_client.')
        future_cart = self.cartesian_client.call_async(cart_req)
        rclpy.spin_until_future_complete(self, future_cart)
        self.get_logger().info('Received cartesian_client result.')
