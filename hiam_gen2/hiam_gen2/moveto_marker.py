...
class MoveIntoHeater(Node):
    def __init__(self):
        ...
        self.declare_parameter('z', 0.210)
    ...
    def marker_listener_callback(self, msg: MarkerPoseWithID):
        ...
        x, y, z, qw, qx, qy, qz = self.compute_target_pose(
                ...
                x_offset=0.0,
                y_offset=0.0,
                z_offset=self.z, #0.204 0.120
        )
        goal_pose = Pose()
        goal_pose.position.x = x
        goal_pose.position.y = y
        goal_pose.position.z = z
        goal_pose.orientation.w = qw #0.58547 #0.7071
        goal_pose.orientation.x = qx #0.012608 #0.0
        goal_pose.orientation.y = qy #0.810592 #0.7071
        goal_pose.orientation.z = qz #0.001656 #0.0
