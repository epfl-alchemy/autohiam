...
cmd_cartesian_move_down_precipitation = (
    "source ~/directory_env/curobo_env/bin/activate && "
    "cd ~/autohiam_ws && source install/setup.bash && "
    "ros2 run hiam_gen2 cartesian_control_moveit --ros-args -p z_offset:=-0.115"
)
...
def infusion():
    ...
    subprocess.run(cmd_move_to_infusion_cover_base, shell=True, executable="/bin/bash", check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
