...
cmd_cartesian_move_down_water = (
    "source ~/directory_env/curobo_env/bin/activate && "
    "cd ~/autohiam_ws && source install/setup.bash && "
    "ros2 run robochemist cartesian_control_moveit --ros-args -p z_offset:=-0.08"
)
...
def washing():
    ...
    print("Moving the sample to the water...")
    subprocess.run(cmd_move_to_water, shell=True, executable="/bin/bash", check=True)

    print("Moving down...")
    subprocess.run(cmd_cartesian_move_down_water, shell=True, executable="/bin/bash", check=True)
    ...
    # print("Picking up the amonia cover...")
