...
cmd_move_to_beginning = (
    "source ~/directory_env/curobo_env/bin/activate && "
    "cd ~/autohiam_ws && source install/setup.bash && "
    "ros2 run robochemist beginning"
)
...
def washing():
    ...
    print("Moving the starting position...")
    subprocess.run(cmd_move_to_beginning, shell=True, executable="/bin/bash", check=True)
