...
cmd_cartesian_move_down_cover_base = (
    "source ~/directory_env/curobo_env/bin/activate && "
    "cd ~/autohiam_ws && source install/setup.bash && "
    "ros2 run robochemist cartesian_control_moveit --ros-args -p z_offset:=-0.08"
)
cmd_cartesian_move_down_sample_heater = (
    "source ~/directory_env/curobo_env/bin/activate && "
    "cd ~/autohiam_ws && source install/setup.bash && "
    "ros2 run robochemist cartesian_control_moveit --ros-args -p z_offset:=-0.08"
)
...
def infusion():
    ...
    print("Moving to cover base...")
    subprocess.run(cmd_move_to_cover_base, shell=True, executable="/bin/bash", check=True)

    print("Moving down...")
    subprocess.run(cmd_cartesian_move_down_cover_base, shell=True, executable="/bin/bash", check=True)
    ...
    subprocess.run(cmd_cartesian_move_down_sample_heater, shell=True, executable="/bin/bash", check=True)
