...
class cuRoboExecNode(Node):
    def __init__(
        ...
        self.pub = self.create_publisher(Bool, 'execution_done', 10)
        self.create_subscription(Bool, 'motion_generated', self.motion_generated_callback, 10)
        self.ready_to_execute = False

    def motion_generated_callback(self, msg):
        if msg.data:
            self.ready_to_execute = True
            self.get_logger().info("New motion generated, ready to execute.")
    ...
    def execute(self, positions: list, velocities: list, accelerations: list, dt):
        ...
        # send goal
        goal_future = self.joint_trajectory_action_client_.send_goal_async(joint_trajectory_goal)
...
def main(args: list = None) -> None:
    ...
    try:
        while rclpy.ok():
            rclpy.spin_once(curobo_exec_node, timeout_sec=0.1)  # Process callbacks
            
            if curobo_exec_node.ready_to_execute:
                positions_path = '/home/szhuang/autohiam_ws/positions_list.yaml'
                velocities_path = '/home/szhuang/autohiam_ws/velocities_list.yaml'
                accelerations_path = '/home/szhuang/autohiam_ws/accelerations_list.yaml'
                dt_path = '/home/szhuang/autohiam_ws/optimized_dt.yaml'

                positions = curobo_exec_node.get_motion(positions_path)
                velocities = curobo_exec_node.get_motion(velocities_path)
                accelerations = curobo_exec_node.get_motion(accelerations_path)
                dt = curobo_exec_node.get_motion(dt_path)

                curobo_exec_node.execute(positions, velocities, accelerations, dt)
            else:
                curobo_exec_node.get_logger().warn("Not ready to execute. Waiting for motion_generated signal.")
                
    except KeyboardInterrupt:
        curobo_exec_node.get_logger().info('Node stopped cleanly')
    finally:
        rclpy.shutdown()
