...
class GripperCloseNode(Node):
    ...
    #     try:
    #         index = msg.name.index("joint7")
    #         self.__js_buffer = {
    #             'position': msg.position[index],
    #         }
    #     except ValueError:
    #         self.get_logger().warn("joint7 not found in joint_states")
...
def main(args: list = None) -> None:
    ...
    gripper_close_node = GripperCloseNode(
        "gripper_close_node"
    )
    ...
    for pos in gripper_positions_degrees:
        gripper_close_node.get_logger().info(f"Moving gripper to position: {pos}")
        gripper_close_node.execute(pos)
